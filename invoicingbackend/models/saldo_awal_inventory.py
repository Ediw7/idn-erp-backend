...
class SaldoAwalInventory(models.Model):
    ...
    gudang_id = fields.Many2one(
        "invoicingbackend.gudang", string="Gudang", required=True
    )
    tanggal = fields.Date(string="Tanggal", required=True)
    keterangan = fields.Char(string="Keterangan")

    line_ids = fields.One2many(
        "invoicingbackend.saldo_awal_inventory_line",
        "saldo_awal_id",
        string="Detail Item",
    )
...
class SaldoAwalInventoryLine(models.Model):
    ...
    saldo_awal_id = fields.Many2one(
        "invoicingbackend.saldo_awal_inventory", string="Saldo Awal", ondelete="cascade"
    )
