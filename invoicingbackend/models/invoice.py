...
class Invoice(models.Model):
    ...
    tgl_invoice = fields.Date(string='Tgl Invoice', required=True, default=fields.Date.context_today)
    ...
    surat_jalan_id = fields.Many2one('invoicingbackend.surat_jalan', string='Dari Surat Jalan (Legacy)')
    surat_jalan_ids = fields.One2many('invoicingbackend.surat_jalan', 'invoice_id', string='Surat Jalan Terkait')
    ...
    # Saldo
    total_terbayar = fields.Float(string='Total Terbayar', compute='_compute_terbayar', store=True)
    saldo_piutang = fields.Float(string='Saldo Piutang', compute='_compute_saldo', store=True)
    ...
    def _compute_terbayar(self):
        for record in self:
            ...
            valid_payments = record.pembayaran_line_ids.filtered(lambda l: not l.pembayaran_id.is_void)
...
class InvoiceLine(models.Model):
    ...
    invoice_id = fields.Many2one('invoicingbackend.invoice', string='Invoice', ondelete='cascade')
    ...
    harga_jual = fields.Float(string='Subtotal Line', compute='_compute_harga_jual', store=True)
