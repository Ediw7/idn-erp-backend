...
class SuratJalan(models.Model):
    ...
    pelanggan_id = fields.Many2one(
        "invoicingbackend.pelanggan", string="Nama Pelanggan", required=True
    )
    ...
    so_id = fields.Many2one(
        "invoicingbackend.sales_order", string="No. Sales Order", ondelete="restrict"
    )
    no_po = fields.Char(string="No. PO")
    no_kendaraan = fields.Char(string="No. Kendaraan")
    no_invoice = fields.Char(string="No. Invoice")
    invoice_id = fields.Many2one(
        "invoicingbackend.invoice", string="Invoice", ondelete="set null"
    )
    ...
    def unlink(self):
        for record in self:
            if (
                record.invoice_id
                or self.env["invoicingbackend.invoice"].search_count(
                    [("surat_jalan_ids", "=", record.id)]
                )
                > 0
            ):
                raise models.ValidationError(
                    "Tidak bisa menghapus Surat Jalan yang sudah ditagihkan (Invoice)."
                )
        ...
        if vals.get("is_void"):
            for record in self:
                if (
                    ...
                    raise models.ValidationError(
                        "Tidak bisa membatalkan (Void) Surat Jalan yang sudah ditagihkan (Invoice)."
                    )
...
class SuratJalanLine(models.Model):
    ...
    sj_id = fields.Many2one(
        "invoicingbackend.surat_jalan", string="Surat Jalan", ondelete="cascade"
    )
    ...
    def _check_kuantum(self):
        for line in self:
            if line.kuantum < 0:
                raise models.ValidationError(
                    "Kuantum (jumlah pengiriman) tidak boleh bernilai negatif."
                )

            if line.sj_id.so_id and line.item_id:
                # Find the total ordered in SO
                so_line = line.sj_id.so_id.line_ids.filtered(
                    lambda l: l.item_id.id == line.item_id.id
                )
                ...
                if total_shipped > total_so_qty:
                    raise models.ValidationError(
                        f"Over-delivery! Anda mengirim {total_shipped} untuk barang {line.item_id.nama}, melebihi pesanan di Sales Order ({total_so_qty})."
                    )
