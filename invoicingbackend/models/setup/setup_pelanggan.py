...
class Pelanggan(models.Model):
    ...
    pembayaran_id = fields.Many2one(
        "invoicingbackend.pembayaran", string="Cara Pembayaran", ondelete="set null"
    )
    tingkat_harga = fields.Selection(
        [("1", "Harga Jual 1"), ("2", "Harga Jual 2"), ("3", "Harga Jual 3")],
        string="Tingkatan Harga Jual",
        default="1",
    )
    diskon = fields.Float(string="Discount Harga Jual (%)", default=0.0)
    perk_piutang_id = fields.Many2one(
        "invoicingbackend.perkiraan", string="No Perkiraan Piutang", ondelete="set null"
    )
