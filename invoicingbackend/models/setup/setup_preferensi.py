...
class SetupPreferensi(models.Model):
    ...
    folder_backup_data = fields.Char(
        string="Folder Backup Data", default="C:\\krishand\\invc\\601"
    )
    folder_csv_faktur = fields.Char(
        string="Folder File CSV e-Faktur", default="C:\\krishand\\invc\\601"
    )
    nama_file_logo = fields.Char(
        string="Nama File Logo", default="C:\\krishand\\invc\\601\\logo.jpg"
    )
    ...
    dokumen_pemotongan_inventory = fields.Selection(
        [("Surat Jalan", "Surat Jalan"), ("Invoice", "Invoice")],
        string="Dokumen Pemotongan Inventory",
        default="Surat Jalan",
    )
    validasi_qty_minus_sj = fields.Boolean(
        string="Validasi Qty Stok Minus di SJ", default=True
    )
    validasi_qty_minus_so = fields.Boolean(
        string="Validasi Qty Stok Minus di SO", default=True
    )
    ...
    selisih_hari_invoice_faktur = fields.Integer(
        string="Selisih Hari Tgl Invoice dgn Faktur Pajak", default=0
    )
    notif_sisa_faktur_kurang_dari = fields.Integer(
        string="Notifikasi Sisa Faktur Kurang Dari", default=5
    )
    ...
    ket_lembar_3_fp = fields.Char(
        string="Ket. Lembar ke-3 FP", default="Lembar Ke-3 : Untuk Arsip"
    )
    ket_lembar_4_fp = fields.Char(
        string="Ket. Lembar ke-4 FP", default="Lembar Ke-4 : Untuk Arsip"
    )
