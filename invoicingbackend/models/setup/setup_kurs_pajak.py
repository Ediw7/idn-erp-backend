        ...
        for record in self:
            if record.tgl_sd and record.tgl_dari and record.tgl_sd < record.tgl_dari:
                raise ValidationError(
                    "Tanggal Selesai (s/d) tidak boleh lebih kecil dari Tanggal Mulai (Dari)."
                )
    ...
    def _check_overlap(self):
        for record in self:
            domain = [
                ...
                "|",
                "&",
                ("tgl_dari", "<=", record.tgl_dari),
                ("tgl_sd", ">=", record.tgl_dari),
                "&",
                ("tgl_dari", "<=", record.tgl_sd),
                ("tgl_sd", ">=", record.tgl_sd),
                "&",
                ("tgl_dari", ">=", record.tgl_dari),
                ("tgl_sd", "<=", record.tgl_sd),
            ]
            ...
            if overlapping:
                raise ValidationError(
                    f"Rentang tanggal kurs pajak bertabrakan dengan kurs yang sudah ada ({overlapping.tgl_dari} s/d {overlapping.tgl_sd})."
                )
