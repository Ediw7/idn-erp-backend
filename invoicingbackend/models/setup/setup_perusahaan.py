...
class ResCompanyExt(models.Model):
    ...
    l10n_id_npwp = fields.Char(
        string="NPWP Perusahaan", size=25, help="Nomor Pokok Wajib Pajak"
    )
    l10n_id_nitku = fields.Char(
        string="NITKU", size=22, help="Nomor Identitas Tempat Kegiatan Usaha"
    )
    ...
    idn_tahun_buku_start = fields.Selection(
        [(str(i), str(i)) for i in range(1, 13)], string="Tahun Buku Awal", default="1"
    )
    idn_tahun_buku_end = fields.Selection(
        [(str(i), str(i)) for i in range(1, 13)],
        string="Tahun Buku Akhir",
        default="12",
    )
    ...
    idn_periode_serial = fields.Char(
        string="Periode Serial", default=_default_periode_serial
    )
