...
class NotaReturPembelian(models.Model):
    ...
    line_ids = fields.One2many(
        "invoicingbackend.nota_retur_pembelian_line", "nota_id", string="Lines"
    )
...
class NotaReturPembelianLine(models.Model):
    ...
    nota_id = fields.Many2one(
        "invoicingbackend.nota_retur_pembelian", string="Nota Retur", ondelete="cascade"
    )
    ...
    harga_jual = fields.Float(
        string="Harga Jual"
    )  # Keeps legacy naming from screenshot
