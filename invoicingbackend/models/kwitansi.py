...
class Kwitansi(models.Model):
    ...
    tgl_kwitansi = fields.Date(
        string="Tgl Kwitansi", required=True, default=fields.Date.context_today
    )
    jenis = fields.Selection(
        [("VAT", "VAT"), ("Non-VAT", "Non-VAT")], string="Jenis Kwitansi", default="VAT"
    )

    invoice_id = fields.Many2one(
        "invoicingbackend.invoice", string="Dari Invoice", ondelete="restrict"
    )
    pelanggan_id = fields.Many2one(
        "invoicingbackend.pelanggan", string="Sudah Terima Dari", required=True
    )

    mata_uang = fields.Selection(
        [("IDR", "IDR"), ("USD", "USD")], string="Mata Uang", default="IDR"
    )
