...
class PembayaranPiutang(models.Model):
    ...
    tgl_pembayaran = fields.Date(string='Tgl Pembayaran', required=True, default=fields.Date.context_today)
    
    pelanggan_id = fields.Many2one('invoicingbackend.pelanggan', string='Dibayar Oleh', required=True)
    
    perkiraan_kas_id = fields.Many2one('invoicingbackend.perkiraan', string='Kas/Bank Penerima')
    keterangan = fields.Text(string='Keterangan')
    
    total_pembayaran = fields.Float(string='Total Pembayaran', compute='_compute_totals', store=True)
    total_potongan = fields.Float(string='Total Potongan', compute='_compute_totals', store=True)
    
    is_void = fields.Boolean(string='Void', default=False)
    
    line_ids = fields.One2many('invoicingbackend.pembayaran_piutang_line', 'pembayaran_id', string='Rincian Pembayaran')
...
class PembayaranPiutangLine(models.Model):
    ...
    perkiraan_potongan_id = fields.Many2one('invoicingbackend.perkiraan', string='Perkiraan Potongan')
