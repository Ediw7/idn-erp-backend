...
class ApiSuratJalan(http.Controller):
    ...
    def get_surat_jalan(self, **kwargs):
        ...
        try:
            ...
            records = request.env["invoicingbackend.surat_jalan"].search(
                domain, order="tgl_sj desc", limit=limit
            )
                ...
                data.append(
                    {
                        ...
                        "no_sj": rec.no_sj or "",
                        "tanggal": (
                            rec.tgl_sj.strftime("%Y-%m-%d") if rec.tgl_sj else ""
                        ),
        ...
        try:
            ...
            if not data.get("no_sj"):
                return ApiResponse.error(
                    message="Nomor Surat Jalan wajib diisi.", status_code=400
                )
            if not data.get("tgl_sj"):
                return ApiResponse.error(
                    message="Tanggal Surat Jalan wajib diisi.", status_code=400
                )
            if not data.get("pelanggan_id"):
                return ApiResponse.error(
                    message="Nama Pelanggan wajib dipilih.", status_code=400
                )
            ...
            existing_sj = request.env["invoicingbackend.surat_jalan"].search(
                domain, limit=1
            )
            if existing_sj:
                return ApiResponse.error(
                    message=f"Nomor Surat Jalan {no_sj} sudah digunakan oleh transaksi lain. Silakan gunakan Auto No atau masukkan nomor unik.",
                    status_code=400,
                )
            ...
            if sj_id:
                ...
                if record.exists():
                    if record.invoice_id or record.no_invoice:
                        return ApiResponse.error(
                            message="Surat Jalan tidak dapat diubah karena sudah memiliki Invoice aktif!",
                            status_code=400,
                        )
                    ...
                    return ApiResponse.error(
                        message="Data Surat Jalan tidak ditemukan", status_code=404
                    )
            else:
                record = request.env["invoicingbackend.surat_jalan"].create(vals)

            return ApiResponse.success(
                data={"id": record.id}, message="Surat Jalan berhasil disimpan"
            )
    ...
    def delete_surat_jalan(self, **kw):
        ...
        try:
            ...
            if not sj_id:
                return ApiResponse.error(
                    message="ID Surat Jalan tidak ditemukan.", status_code=400
                )
            record = request.env["invoicingbackend.surat_jalan"].browse(int(sj_id))
            if record.exists():
                if record.invoice_id or record.no_invoice:
                    return ApiResponse.error(
                        message="Surat Jalan tidak dapat dihapus karena sudah memiliki Invoice aktif!",
                        status_code=400,
                    )
