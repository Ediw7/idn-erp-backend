...
class ApiSaldoAwalInventory(http.Controller):
    ...
    def get_saldo_awal(self, **kw):
        try:
            records = request.env["invoicingbackend.saldo_awal_inventory"].search(
                ...
                data.append(
                    {
                        ...
                        "gudang_nama": (
                            rec.gudang_id.nama_gudang if rec.gudang_id else ""
                        ),
    ...
    def save_saldo_awal(self, **kw):
        try:
            ...
            if record_id:
                existing_record = request.env[
                    "invoicingbackend.saldo_awal_inventory"
                ].browse(record_id)
                existing_line_ids = existing_record.line_ids.ids
                incoming_line_ids = [
                    l.get("id") for l in params.get("lines", []) if l.get("id")
                ]
            ...
            if record_id:
                record = request.env["invoicingbackend.saldo_awal_inventory"].browse(
                    record_id
                )
                record.write(vals)
            else:
                vals["company_id"] = request.env.user.company_id.id
                record = request.env["invoicingbackend.saldo_awal_inventory"].create(
                    vals
                )
