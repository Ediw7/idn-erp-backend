    ...
    def get_sales_order(self, **kw):
        try:
            ...
            for rec in records:
                ...
                surat_jalans = request.env["invoicingbackend.surat_jalan"].search(
                    [("so_id", "=", rec.id)]
                )
                sj_list = []
                for sj in surat_jalans:
                    ...
                    for sj_line in sj.line_ids:
                        sj_lines.append(
                            {
                                "item_id": (
                                    sj_line.item_id.id if sj_line.item_id else None
                                ),
                ...
                data.append(
                    {
                        ...
                        "pelanggan_alamat_kirim": (
                            rec.pelanggan_id.alamat_kirim if rec.pelanggan_id else ""
                        ),
                        "alamat_kirim": rec.alamat_kirim or "",
                        "no_po": rec.no_po or "",
                        "tgl_po": rec.tgl_po.strftime("%Y-%m-%d") if rec.tgl_po else "",
                        "mata_uang_id": (
                            rec.mata_uang_id.id if rec.mata_uang_id else None
                        ),
                        "mata_uang_kode": (
                            rec.mata_uang_id.kode if rec.mata_uang_id else ""
                        ),
                        "pembayaran_id": (
                        ...
                        "salesman_nama": (
                            rec.salesman_id.nama if rec.salesman_id else ""
                        ),
                        "tgl_kirim": (
                            rec.tgl_kirim.strftime("%Y-%m-%d") if rec.tgl_kirim else ""
                        ),
                        ...
                        "total": rec.total,
                        "create_date": (
                            rec.create_date.strftime("%m/%d/%Y %I:%M:%S %p")
                            if rec.create_date
                            else ""
                        ),
                        "create_uid_name": (
                            rec.create_uid.name if rec.create_uid else ""
                        ),
                        "write_date": (
                            rec.write_date.strftime("%m/%d/%Y %I:%M:%S %p")
                            if rec.write_date
                            else ""
                        ),
    ...
    def save_sales_order(self, **kw):
        try:
            ...
            vals = {
                ...
                "pelanggan_id": (
                    int(params["pelanggan_id"]) if params.get("pelanggan_id") else False
                ),
                "alamat_kirim": params.get("alamat_kirim", ""),
                "no_po": params.get("no_po", ""),
                "tgl_po": params.get("tgl_po") or False,
                "mata_uang_id": (
                    int(params["mata_uang_id"]) if params.get("mata_uang_id") else False
                ),
                "pembayaran_id": (
                    int(params["pembayaran_id"])
                    if params.get("pembayaran_id")
                    else False
                ),
                "salesman_id": (
                    int(params["salesman_id"]) if params.get("salesman_id") else False
                ),
            ...
            if record_id:
                record = request.env["invoicingbackend.sales_order"].browse(
                    int(record_id)
                )
                if record.exists():
                    has_sj = (
                        request.env["invoicingbackend.surat_jalan"].search_count(
                            [("so_id", "=", record.id)]
                        )
                        > 0
                    )
                    has_inv = (
                        request.env["invoicingbackend.invoice"].search_count(
                    ...
                    if has_sj or has_inv:
                        return {
                            "status": "error",
                            "message": "Sales Order tidak dapat diedit karena sudah memiliki relasi ke Surat Jalan atau Invoice!",
                        }
        ...
        try:
            ...
            if record.exists():
                ...
                if has_sj or has_inv:
                    return {
                        "status": "error",
                        "message": "Sales Order tidak dapat dihapus karena sudah memiliki relasi ke Surat Jalan atau Invoice!",
                    }
