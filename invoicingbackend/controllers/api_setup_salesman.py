...
class ApiSetupSalesman(http.Controller):
    ...
    @http.route('/api/setup/salesman/get', type='json', auth='user', methods=['POST'], cors='*')
    def get_salesman(self, **kw):
        try:
            records = request.env['invoicingbackend.salesman'].search([], order='kode asc')
            data = []
            for rec in records:
                data.append({
                    'id': rec.id,
    ...
    @http.route('/api/setup/salesman/save', type='json', auth='user', methods=['POST'], cors='*')
    def save_salesman(self, **kw):
        try:
            ...
            if record_id:
                record = request.env['invoicingbackend.salesman'].browse(record_id)
                if record.exists():
                    record.write(vals)
                else:
    ...
    @http.route('/api/setup/salesman/delete', type='json', auth='user', methods=['POST'], cors='*')
