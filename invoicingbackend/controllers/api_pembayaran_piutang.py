...
class ApiPembayaranPiutang(http.Controller):
    ...
    def get_pembayaran(self, **kwargs):
        ...
        try:
            ...
            records = request.env["invoicingbackend.pembayaran_piutang"].search(
                domain, order="tgl_pembayaran desc", limit=limit
            )
                    ...
                    lines.append(
                        {
                            ...
                            "no_invoice": (
                                line.invoice_id.no_invoice if line.invoice_id else ""
                            ),
                            "no_faktur_pajak": (
                                line.invoice_id.no_fp if line.invoice_id else ""
                            ),
                            "tgl_jt": "",
                            "ccy": "IDR",
                            "saldo_piutang": (
                                line.invoice_id.saldo_piutang + line.pembayaran
                                if line.invoice_id
                                else 0
                            ),  # rough estimate before save
                ...
                data.append(
                    {
                        ...
                        "tanggal": (
                            str(rec.tgl_pembayaran) if rec.tgl_pembayaran else ""
                        ),
                        ...
                        or "",
                        "perkiraan_kas_bank": (
                            rec.perkiraan_kas_id.id if rec.perkiraan_kas_id else ""
                        ),
        ...
        try:
            ...
            if not no_bukti:
                return ApiResponse.error(
                    message="Nomor Bukti wajib diisi.", status_code=400
                )

            domain = [("no_bukti", "=", no_bukti)]
            if pem_id:
                domain.append(("id", "!=", int(pem_id)))
            existing = request.env["invoicingbackend.pembayaran_piutang"].search(
                domain, limit=1
            )
            if existing:
                return ApiResponse.error(
                    message=f"Nomor Bukti {no_bukti} sudah digunakan. Silakan gunakan Auto No.",
                    status_code=400,
                )
            ...
            if pem_id:
                record = request.env["invoicingbackend.pembayaran_piutang"].browse(
                    pem_id
                )
                if record.exists():
                    if record.is_void:
                        return ApiResponse.error(
                            message="Tidak dapat merubah pembayaran yang sudah di-void",
                            status_code=400,
                        )
                    record.write(vals)
                else:
                    return ApiResponse.error(
                        message="Data Pembayaran tidak ditemukan", status_code=404
                    )
            else:
                record = request.env["invoicingbackend.pembayaran_piutang"].create(vals)

            return ApiResponse.success(
                data={"id": record.id}, message="Pembayaran berhasil disimpan"
            )
    ...
    def delete_pembayaran(self, **kw):
        ...
        try:
            ...
            if not pem_id:
                return ApiResponse.error(
                    message="ID Pembayaran tidak ditemukan.", status_code=400
                )
            record = request.env["invoicingbackend.pembayaran_piutang"].browse(
                int(pem_id)
            )
            if record.exists():
                if record.is_void:
                    return ApiResponse.error(
                        message="Pembayaran sudah di-void tidak dapat dihapus",
                        status_code=400,
                    )
                record.unlink()
