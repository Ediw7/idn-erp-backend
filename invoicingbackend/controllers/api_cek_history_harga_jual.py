...
class ApiCekHistoryHargaJual(http.Controller):
    ...
    @http.route('/api/history-harga-jual/get', type='json', auth='user', methods=['POST'], cors='*')
    def get_history(self, **kwargs):
        try:
            ...
            domain = [('invoice_id.company_id', '=', request.env.user.company_id.id), ('invoice_id.is_void', '=', False)]
            ...
            for line in line_records:
                ...
                if inv.pelanggan_id and inv.pelanggan_id.pembayaran_id:
                    terms = str(inv.pelanggan_id.pembayaran_id.hari_jatuh_tempo) + ' Hari'
                ...
                data.append({
                    ...
                    'nama_pelanggan': inv.pelanggan_id.nama if inv.pelanggan_id else '',
                    'terms': terms,
                    'curr': curr,
                    'kode_item': line.item_id.kode if line.item_id else '',
                    'nama_item': line.item_id.nama if line.item_id else line.nama_barang or '',
                    'qty': line.kuantum,
