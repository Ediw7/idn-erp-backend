    ...
    def get_kartu_piutang(self, **kwargs):
        ...
        try:
            ...
            invoices = (
                ...
                .sudo()
                .search([("pelanggan_id", "=", pelanggan_id)])
            )
            for inv in invoices:
                ...
                riwayat_list.append(
                    {
                        "id": f"inv_{inv.id}",
                        "tanggal": (
                            str(inv.tanggal_faktur) if inv.tanggal_faktur else ""
                        ),
            ...
            pembayaran_lines = (
                request.env["invoicingbackend.pembayaran_piutang_line"]
                .sudo()
                .search(
                    [
                ...
                riwayat_list.append(
                    {
                        "id": f"pay_{line.id}",
                        "tanggal": (
                            str(line.pembayaran_id.tgl_pembayaran)
                            if line.pembayaran_id.tgl_pembayaran
                            else ""
                        ),
                        "no_invoice": (
                        ...
                        or line.pembayaran_id.keterangan
                        or "Pembayaran Piutang",
                        "debet": 0.0,
                        "kredit": total_kredit,
                    }
                )
            ...
            nota_kredits = (
                request.env["invoicingbackend.nota_kredit"]
                .sudo()
                .search([("pelanggan_id", "=", pelanggan_id)])
            )
                ...
                riwayat_list.append(
                    {
                        "id": f"nk_{nk.id}",
                        "tanggal": (
                            str(nk.tgl_nota_kredit) if nk.tgl_nota_kredit else ""
                        ),
            ...
            resp = {"status": "success", "data": riwayat_list}
