...
class ApiNotaKredit(http.Controller):
    ...
    def get_all_nota_kredit(self, **kwargs):
        ...
        try:
            ...
            if page:
                page = int(page)
                offset = (page - 1) * limit
                records = nota_kredit_obj.search(
                ...
                data.append(
                    {
                        ...
                        "tgl_nota_kredit": (
                            r.tgl_nota_kredit.strftime("%Y-%m-%d")
                            if r.tgl_nota_kredit
                            else None
                        ),
                        "periode": r.periode,
                        "pelanggan_id": r.pelanggan_id.id if r.pelanggan_id else None,
                        "pelanggan_nama": (
                            r.pelanggan_id.nama if r.pelanggan_id else None
                        ),
                        "alamat": r.alamat,
                        ...
                        "mata_uang_nama": (
                            r.mata_uang_id.nama if r.mata_uang_id else None
                        ),
                        "nilai_nota_kredit": r.nilai_nota_kredit,
                        "tanda_tangan": r.tanda_tangan,
                        "jabatan": r.jabatan,
                        "create_date": (
                            r.create_date.strftime("%Y-%m-%d %H:%M:%S")
                            if r.create_date
                            else None
                        ),
            ...
            return ApiResponse.success(
                message="Data Nota Kredit berhasil diambil",
                data=data,
    ...
    def get_nota_kredit_by_id(self, nk_id, **kwargs):
        ...
        try:
            ...
            data = {
                ...
                "tgl_nota_kredit": (
                    record.tgl_nota_kredit.strftime("%Y-%m-%d")
                    if record.tgl_nota_kredit
                    else None
                ),
                "periode": record.periode,
                "pelanggan_id": record.pelanggan_id.id if record.pelanggan_id else None,
                "pelanggan_nama": (
                    record.pelanggan_id.nama if record.pelanggan_id else None
                ),
                "alamat": record.alamat,
                "invoice_id": record.invoice_id.id if record.invoice_id else None,
                "no_invoice": (
                    record.invoice_id.no_invoice if record.invoice_id else None
                ),
            ...
            return ApiResponse.success(
                message="Detail Nota Kredit berhasil diambil", data=data
            )
        except Exception as e:
            return ApiResponse.error(str(e))
    ...
    def create_nota_kredit(self, **kwargs):
        ...
        try:
            ...
            vals = {
                ...
                "pelanggan_id": (
                    int(payload.get("pelanggan_id"))
                    if payload.get("pelanggan_id")
                    else False
                ),
                "invoice_id": (
                    int(payload.get("invoice_id"))
                    if payload.get("invoice_id")
                    else False
                ),
                "no_referensi": payload.get("no_referensi"),
                "mata_uang_id": (
                    int(payload.get("mata_uang_id"))
                    if payload.get("mata_uang_id")
                    else False
                ),
            ...
            return ApiResponse.success(
                message="Nota Kredit berhasil dibuat",
                data={"id": new_record.id, "no_nota_kredit": new_record.no_nota_kredit},
            )
        except Exception as e:
            return ApiResponse.error(str(e))
    ...
    def update_nota_kredit(self, nk_id, **kwargs):
        ...
        try:
            ...
            if "pelanggan_id" in payload:
                vals["pelanggan_id"] = (
                    int(payload["pelanggan_id"]) if payload["pelanggan_id"] else False
                )
            if "invoice_id" in payload:
                vals["invoice_id"] = (
                    int(payload["invoice_id"]) if payload["invoice_id"] else False
                )
            if "no_referensi" in payload:
                vals["no_referensi"] = payload["no_referensi"]
            if "mata_uang_id" in payload:
                vals["mata_uang_id"] = (
                    int(payload["mata_uang_id"]) if payload["mata_uang_id"] else False
                )
            ...
            record.write(vals)
            return ApiResponse.success(
                message="Nota Kredit berhasil diupdate", data={"id": record.id}
            )
        except Exception as e:
            return ApiResponse.error(str(e))
    ...
    def get_nota_kredit_auto_no(self, **kwargs):
        ...
        try:
            ...
            setup = request.env["invoicingbackend.format_bukti"].search(
                [("periode", "=", periode)], limit=1
            )
            ...
            if setup:
                if setup.nota_kredit_prefiks:
                    prefix = setup.nota_kredit_prefiks
                if setup.nota_kredit_sufiks:
                    suffix = setup.nota_kredit_sufiks
                if setup.nota_kredit_digit:
                    digit = int(setup.nota_kredit_digit)

            # Find latest
            domain = [("no_nota_kredit", "ilike", prefix + "%" + suffix)]
            last_record = request.env["invoicingbackend.nota_kredit"].search(
                domain, order="no_nota_kredit desc", limit=1
            )
            ...
            return ApiResponse.success(
                message="Auto No berhasil digenerate", data={"auto_no": auto_no}
            )
        except Exception as e:
            return ApiResponse.error(str(e))
