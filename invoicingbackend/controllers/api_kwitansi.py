...
class ApiKwitansi(http.Controller):
    ...
    def get_kwitansi(self, **kwargs):
        ...
        try:
            ...
            records = request.env["invoicingbackend.kwitansi"].search(
                domain, order="tgl_kwitansi desc", limit=limit
            )
                ...
                data.append(
                    {
                        ...
                        "tgl_kwitansi": (
                            str(rec.tgl_kwitansi) if rec.tgl_kwitansi else ""
                        ),
                        "jenis": rec.jenis or "",
                        "invoice_id": rec.invoice_id.id if rec.invoice_id else "",
                        "no_invoice": (
                            rec.invoice_id.no_invoice if rec.invoice_id else ""
                        ),
        ...
        try:
            ...
            if not no_kwitansi:
                return ApiResponse.error(
                    message="Nomor Kwitansi wajib diisi.", status_code=400
                )
            ...
            if existing:
                return ApiResponse.error(
                    message=f"Nomor Kwitansi {no_kwitansi} sudah digunakan. Silakan gunakan Auto No.",
                    status_code=400,
                )
            ...
            if kw_id:
                ...
                if record.exists():
                    ...
                    return ApiResponse.error(
                        message="Data Kwitansi tidak ditemukan", status_code=404
                    )
            else:
                record = request.env["invoicingbackend.kwitansi"].create(vals)

            return ApiResponse.success(
                data={"id": record.id}, message="Kwitansi berhasil disimpan"
            )
    ...
    def delete_kwitansi(self, **kw):
        ...
        try:
            ...
            if not kw_id:
                return ApiResponse.error(
                    message="ID Kwitansi tidak ditemukan.", status_code=400
                )
