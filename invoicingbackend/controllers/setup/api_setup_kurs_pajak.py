...
class ApiSetupKursPajak(http.Controller):
    ...
    @http.route(
        "/api/setup/kurspajak/get", type="json", auth="user", methods=["POST"], cors="*"
    )
    def get_kurs_pajak(self, **kw):
        try:
            ...
            records = request.env["invoicingbackend.kurs_pajak"].search(
                [("mata_uang_id", "=", int(mata_uang_id))], order="tgl_dari desc"
            )
            data = []
            for rec in records:
                data.append(
    ...
    def save_kurs_pajak(self, **kw):
        try:
            ...
            return {
                "status": "success",
                "message": "Kurs Pajak berhasil disimpan",
                "id": record.id,
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
        ...
        try:
            ...
            if record_id:
                ...
                if record.exists():
                    ...
                    return {
                        "status": "success",
                        "message": "Kurs Pajak berhasil dihapus",
                    }
                else:
                    return {"status": "error", "message": "Data tidak ditemukan"}
