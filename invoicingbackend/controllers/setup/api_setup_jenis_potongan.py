...
class ApiSetupJenisPotongan(http.Controller):
    ...
    def get_jenis_potongan(self, **kw):
        try:
            records = request.env["invoicingbackend.jenis_potongan"].search(
                [], order="kode asc"
            )
            data = []
            for rec in records:
                data.append(
                    {
                        ...
                        "nama": rec.nama or "",
                        "perkiraan_id": (
                            rec.perkiraan_id.id if rec.perkiraan_id else None
                        ),
                        "perkiraan_nama": (
                            f"{rec.perkiraan_id.no_perkiraan} - {rec.perkiraan_id.nama_perkiraan}"
                            if rec.perkiraan_id
                            else ""
                        ),
                    }
    ...
    def save_jenis_potongan(self, **kw):
        try:
            ...
            if record_id:
                record = request.env["invoicingbackend.jenis_potongan"].browse(
                    record_id
                )
                if record.exists():
                    record.write(vals)
        ...
        try:
            ...
            if record_id:
                ...
                if record.exists():
                    ...
                    return {
                        "status": "success",
                        "message": "Jenis Potongan berhasil dihapus",
                    }
                else:
                    return {"status": "error", "message": "Data tidak ditemukan"}
