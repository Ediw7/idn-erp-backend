...
class ApiSetupPelanggan(http.Controller):
    ...
    def get_pelanggan(self, **kw):
        try:
            records = request.env["invoicingbackend.pelanggan"].search(
                [], order="kode asc"
            )
            data = []
            for rec in records:
                data.append(
                    {
                        ...
                        "perk_piutang_id": (
                            rec.perk_piutang_id.id if rec.perk_piutang_id else None
                        ),
                        "perk_piutang_nama": (
                            rec.perk_piutang_id.no_perkiraan
                            if rec.perk_piutang_id
                            else ""
                        ),
    ...
    def import_batch_pelanggan(self, **kw):
        try:
            ...
            for item in items:
                ...
                existing = env_pelanggan.search(
                    [("kode", "=", item.get("kode"))], limit=1
                )
