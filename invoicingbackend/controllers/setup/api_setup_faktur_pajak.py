...
class ApiSetupFakturPajak(http.Controller):
    ...
    def get_faktur_pajak(self, **kw):
        try:
            records = request.env["invoicingbackend.faktur_pajak"].search(
                [], order="tgl_awal desc"
            )
            data = []
            for rec in records:
    ...
    def save_faktur_pajak(self, **kw):
        try:
            ...
            return {
                "status": "success",
                "message": "Setup Faktur Pajak berhasil disimpan",
                "id": record.id,
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
        ...
        try:
            ...
            if record_id:
                ...
                if record.exists():
                    ...
                    return {
                        "status": "success",
                        "message": "Setup Faktur Pajak berhasil dihapus",
                    }
                else:
                    return {"status": "error", "message": "Data tidak ditemukan"}
