...
class ApiSetupFormatBukti(http.Controller):
    ...
    def get_format_bukti(self, **kw):
        try:
            records = request.env["invoicingbackend.format_bukti"].search(
                [], order="periode desc"
            )
            data = []
            for rec in records:
    ...
    def save_format_bukti(self, **kw):
        try:
            ...
            return {
                "status": "success",
                "message": "Format No Bukti berhasil disimpan",
                "id": record.id,
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
        ...
        try:
            ...
            if record_id:
                ...
                if record.exists():
                    ...
                    return {
                        "status": "success",
                        "message": "Format No Bukti berhasil dihapus",
                    }
                else:
                    return {"status": "error", "message": "Data tidak ditemukan"}
