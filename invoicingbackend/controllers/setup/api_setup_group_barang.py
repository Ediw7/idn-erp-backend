...
class ApiSetupGroupBarang(http.Controller):
    ...
    def get_group_barang(self, **kw):
        try:
            records = request.env["invoicingbackend.group_barang"].search(
                [], order="nama asc"
            )
            data = []
            for rec in records:
    ...
    def save_group_barang(self, **kw):
        try:
            ...
            return {
                "status": "success",
                "message": "Group Barang berhasil disimpan",
                "id": record.id,
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
        ...
        try:
            ...
            if record_id:
                ...
                if record.exists():
                    ...
                    return {
                        "status": "success",
                        "message": "Group Barang berhasil dihapus",
                    }
                else:
                    return {"status": "error", "message": "Data tidak ditemukan"}
