...
class ApiSetupBahasa(http.Controller):
    ...
    def get_bahasa(self, **kw):
        try:
            records = request.env["invoicingbackend.bahasa"].search(
                [], order="jenis_objek asc, nama_objek asc, id asc"
            )
            data = []
            for rec in records:
                data.append(
    ...
    def save_bahasa(self, **kw):
        try:
            ...
            return {
                "status": "success",
                "message": "Setup Bahasa berhasil disimpan",
                "id": record.id,
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
        ...
        try:
            ...
            if record_id:
                ...
                if record.exists():
                    ...
                    return {
                        "status": "success",
                        "message": "Setup Bahasa berhasil dihapus",
                    }
                else:
                    return {"status": "error", "message": "Data tidak ditemukan"}
