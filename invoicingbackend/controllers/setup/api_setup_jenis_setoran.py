...
class ApiSetupJenisSetoran(http.Controller):
    ...
    def get_jenis_setoran(self, **kw):
        try:
            records = request.env["invoicingbackend.jenis_setoran"].search(
                [], order="jenis_pajak_id asc, kode asc"
            )
            data = []
            for rec in records:
                data.append(
                    {
                        "id": rec.id,
                        "jenis_pajak_id": (
                            rec.jenis_pajak_id.id if rec.jenis_pajak_id else None
                        ),
                        "jenis_pajak_kode": (
                            rec.jenis_pajak_id.kode if rec.jenis_pajak_id else ""
                        ),
                        "jenis_pajak_nama": (
                            rec.jenis_pajak_id.nama if rec.jenis_pajak_id else ""
                        ),
                        "kode": rec.kode or "",
                        "nama": rec.nama or "",
                    }
    ...
    def save_jenis_setoran(self, **kw):
        try:
            ...
            return {
                "status": "success",
                "message": "Kode Jenis Setoran berhasil disimpan",
                "id": record.id,
            }
        except Exception as e:
            return {"status": "error", "message": str(e)}
    ...
    @http.route(
        "/api/setup/jenis_setoran/delete",
        type="json",
        auth="user",
        methods=["POST"],
        cors="*",
        ...
        try:
            ...
            if record_id:
                ...
                if record.exists():
                    ...
                    return {
                        "status": "success",
                        "message": "Kode Jenis Setoran berhasil dihapus",
                    }
                else:
                    return {"status": "error", "message": "Data tidak ditemukan"}
