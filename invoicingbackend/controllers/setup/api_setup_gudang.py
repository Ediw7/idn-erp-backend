...
class ApiSetupGudang(http.Controller):
    ...
    def get_gudang(self, **kw):
        try:
            records = request.env["invoicingbackend.gudang"].search(
                [], order="kode_gudang asc"
            )
            data = []
            for rec in records:
                data.append(
    ...
    def save_gudang(self, **kw):
        try:
            ...
            if is_default:
                other_defaults = request.env["invoicingbackend.gudang"].search(
                    [("is_default", "=", True)]
                )
                if record_id:
                    other_defaults = other_defaults.filtered(
                        lambda r: r.id != record_id
                    )
