...
class ApiSetupItem(http.Controller):
    ...
    @http.route(
        "/api/setup/item/get", type="json", auth="user", methods=["POST"], cors="*"
    )
    def get_item(self, **kw):
        try:
            ...
            if params.get("group_barang_id"):
                domain.append(
                    ("group_barang_id", "=", int(params.get("group_barang_id")))
                )

            records = request.env["invoicingbackend.item"].search(
                domain, order="kode asc"
            )
            data = []
            for rec in records:
                data.append(
                    {
                        ...
                        "nama": rec.nama,
                        "group_barang_id": (
                            rec.group_barang_id.id if rec.group_barang_id else None
                        ),
                        "group_barang_nama": (
                            rec.group_barang_id.nama if rec.group_barang_id else ""
                        ),
                        ...
                        "perk_penjualan_id": (
                            rec.perk_penjualan_id.id if rec.perk_penjualan_id else None
                        ),
                        "perk_penjualan_nama": (
                            rec.perk_penjualan_id.no_perkiraan
                            if rec.perk_penjualan_id
                            else ""
                        ),
                        "perk_hpp_id": rec.perk_hpp_id.id if rec.perk_hpp_id else None,
                        "perk_hpp_nama": (
                            rec.perk_hpp_id.no_perkiraan if rec.perk_hpp_id else ""
                        ),
    ...
    @http.route(
        "/api/setup/item/save", type="json", auth="user", methods=["POST"], cors="*"
    )
    ...
    @http.route(
        "/api/setup/item/delete", type="json", auth="user", methods=["POST"], cors="*"
    )
