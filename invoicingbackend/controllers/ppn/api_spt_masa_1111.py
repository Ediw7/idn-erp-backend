                ...
                data.append(
                    {
                        ...
                        "tanggal_spt": (
                            r.tanggal_spt.strftime("%Y-%m-%d") if r.tanggal_spt else ""
                        ),
                        ...
                        "tgl_lunas_kurang_bayar": (
                            r.tgl_lunas_kurang_bayar.strftime("%Y-%m-%d")
                            if r.tgl_lunas_kurang_bayar
                            else ""
                        ),
    ...
    def create_spt_masa(self, **kw):
        try:
            ...
            val = {
                ...
                "pajak_masukan_diperhitungkan": params.get(
                ...
                "lebih_bayar_diminta_untuk": params.get("lebih_bayar_diminta_untuk")
                or False,
            ...
            if params.get("id"):
                record = request.env["invoicingbackend.spt_masa_1111"].browse(
                    params["id"]
                )
                record.write(val)
                res_id = record.id
