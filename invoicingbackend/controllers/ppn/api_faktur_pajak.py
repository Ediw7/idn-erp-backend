...
class ApiFakturPajak(http.Controller):
    ...
    @http.route('/api/faktur_pajak/get', type='json', auth='public', methods=['POST'], csrf=False)
    def get_faktur_pajak(self, **kw):
        try:
            ...
            records = request.env['invoicingbackend.transaksi_faktur_pajak'].sudo().search(domain, order='tgl_fp desc, id desc', limit=2000)
    ...
    @http.route('/api/faktur_pajak/save', type='json', auth='public', methods=['POST'], csrf=False)
    def save_faktur_pajak(self, **kw):
        try:
            ...
            if fp_id:
                rec = request.env['invoicingbackend.transaksi_faktur_pajak'].sudo().browse(fp_id)
                if rec.exists():
                    rec.write(vals)
                else:
                    rec = request.env['invoicingbackend.transaksi_faktur_pajak'].sudo().create(vals)
            else:
                rec = request.env['invoicingbackend.transaksi_faktur_pajak'].sudo().create(vals)
            ...
            for line_data in lines_data:
                ...
                if line_id and line_id in existing_line_ids:
                    request.env['invoicingbackend.transaksi_faktur_pajak_line'].sudo().browse(line_id).write(line_vals)
                    new_line_ids.append(line_id)
                else:
                    new_line = request.env['invoicingbackend.transaksi_faktur_pajak_line'].sudo().create(line_vals)
                    new_line_ids.append(new_line.id)
            
            # Remove deleted lines
            lines_to_delete = set(existing_line_ids) - set(new_line_ids)
            if lines_to_delete:
                request.env['invoicingbackend.transaksi_faktur_pajak_line'].sudo().browse(list(lines_to_delete)).unlink()
    ...
    @http.route('/api/faktur_pajak/delete', type='json', auth='public', methods=['POST'], csrf=False)
