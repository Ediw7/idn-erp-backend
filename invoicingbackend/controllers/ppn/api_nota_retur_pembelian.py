...
class ApiNotaReturPembelian(http.Controller):
                ...
                data.append(
                    {
                        ...
                        "tgl_nota": (
                            r.tgl_nota.strftime("%Y-%m-%d") if r.tgl_nota else ""
                        ),
    ...
    def create_nota_retur_pembelian(self, **kw):
        try:
            ...
            if not params.get("lines") or len(params["lines"]) == 0:
                return {"status": "error", "message": "Nota Retur Pembelian wajib memiliki minimal 1 barang (lines)."}
            ...
            if params.get("id"):
                record = request.env["invoicingbackend.nota_retur_pembelian"].browse(
                    params["id"]
                )
                ...
                record = request.env["invoicingbackend.nota_retur_pembelian"].create(
                    val
                )
                res_id = record.id
    ...
    def delete_nota_retur_pembelian(self, **kw):
        try:
            record_id = kw.get("id")
            if record_id:
                request.env["invoicingbackend.nota_retur_pembelian"].browse(
                    record_id
                ).unlink()
