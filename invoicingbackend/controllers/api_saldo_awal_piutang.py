...
class ApiSaldoAwalPiutang(http.Controller):
                ...
                data.append(
                    {
                        ...
                        "pelanggan_id": (
                            rec.pelanggan_id.id if rec.pelanggan_id else False
                        ),
                        "pelanggan_name": (
                        ...
                        "mata_uang_id": (
                            rec.mata_uang_id.id if rec.mata_uang_id else False
                        ),
                        "mata_uang_name": (
    ...
    def create_saldo_awal_piutang(self, **kw):
        ...
        try:
            ...
            # Validation
            if (
                not data.get("no_invoice")
                or not data.get("tanggal")
                or not data.get("pelanggan_id")
            ):
                return ApiResponse.error(
                    message="No Invoice, Tanggal, dan Pelanggan harus diisi!"
                )
            ...
            val = {
                ...
                "mata_uang_id": (
                    data.get("mata_uang_id") if data.get("mata_uang_id") else False
                ),
            ...
            return ApiResponse.success(
                message="Data berhasil disimpan", data={"id": new_record.id}
            )
        except Exception as e:
            return ApiResponse.error(message=str(e))
    ...
    def update_saldo_awal_piutang(self, **kw):
        ...
        try:
            ...
            record = request.env["invoicingbackend.saldo_awal_piutang"].browse(
                record_id
            )
            if record.exists():
                val = {}
                if "no_invoice" in data:
                    val["no_invoice"] = data["no_invoice"]
                if "tanggal" in data:
                    val["tanggal"] = data["tanggal"]
                if "pelanggan_id" in data:
                    val["pelanggan_id"] = data["pelanggan_id"]
                if "proyek_id" in data:
                    val["proyek_id"] = data["proyek_id"] if data["proyek_id"] else False
                if "tgl_jt" in data:
                    val["tgl_jt"] = data["tgl_jt"] if data["tgl_jt"] else False
                if "mata_uang_id" in data:
                    val["mata_uang_id"] = (
                        data["mata_uang_id"] if data["mata_uang_id"] else False
                    )
                if "saldo_invoice" in data:
                    val["saldo_invoice"] = float(data["saldo_invoice"] or 0.0)
                if "is_paid" in data:
                    val["is_paid"] = data["is_paid"]
